      ...
      <div className="audit-columns">
        ...
        <div className="audit-right-column">
          ...
          <section className="card free-purchase-card">
            <div className="card-heading"><div><h2>Livre para compra por mês</h2><p>Comparação diária entre os débitos importados e a meta do dia.</p></div><CalendarDays size={21} /></div>
            {comparison.isLoading ? <div className="empty">Calculando disponibilidade mensal...</div> : <div className="goals-table-wrap"><table className="goals-table"><thead><tr><th>Mês</th><th>Orçamento de compra</th><th>Total a pagar</th><th>Livre para compra</th><th>Limite ultrapassado</th></tr></thead><tbody>{comparison.data?.monthlyBudgets.map((item) => <tr key={item.month}><td><strong>{monthLabel(item.month)}</strong></td><td>{money(item.budgetCents / 100)}</td><td>{money(item.totalDebitCents / 100)}</td><td style={{ color: item.blocked ? "#b42318" : "#147d3f", fontWeight: 700 }}>{item.blocked ? "Bloqueado" : money(item.availableCents / 100)}</td><td style={{ color: "#b42318", fontWeight: 700 }}>{item.blocked ? "Bloqueado" : money(item.exceededCents / 100)}</td></tr>)}</tbody></table></div>}
          </section>
        </div>
      </div>
