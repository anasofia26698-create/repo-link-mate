      ...
      <div className="audit-columns">
        ...
        <div className="audit-right-column">
          ...
          <section className="card free-purchase-card">
            ...
            {comparison.data?.automaticRecovery.map((item) => <p key={item.month} className="red-text">Ajuste automático: ultrapassado de {money(item.totalExceededCents / 100)} ({(item.pct * 100).toFixed(2).replace(".", ",")}%) redistribuído na meta diária dos dias restantes do dia {dateBR(item.lastExceededDate)}.</p>)}
          </section>
        </div>
      </div>
